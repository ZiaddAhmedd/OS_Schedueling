...
int main(int argc, char *argv[])
{
    ...
    while (FinishedProcesses < processCount )
    {
        ...
        if (nextTime >= time)
        {
            ...
            {
                //printf("I'm process %d and I'm waiting\n", CurrentRunning->processId);
                ...
                else // it is the first time for the process to run
                {
                    ...
                    //printf("array of pids is now holding %d at position %d\n", arrPids[PidCount], PidCount);
                    //PidCount++;
                }
            }
            ...
            if (CurrentRunning->runTime > quantum)
            {
                ...
                {
                    //printf("killing pid of process id number %d\n",data.processId);
                    //printf("getClk() - CurrentRunning->startTime (%d - %d) = 1\n", getClk(),CurrentRunning->startTime);
